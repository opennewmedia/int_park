...
;(function($) {
	$.widget("wgm.imgNotes", $.wgm.imgViewer, {
		options: {
			...
			onEdit: $.noop,
			onShow: $.noop,
			markerImg: "typo3conf/ext/int_park/Resources/Public/lib/images/marker-icon.png",
/*
 ...
 */
			onAdd: function() {
				...
				if(notes[this.notes.length]) {
					var elem = $(document.createElement('div')).addClass("marker").css({'width':settings.pinW,'height':settings.pinH}).prepend('<div class="marker" style="background-image: url(\'../typo3conf/ext/int_park/Resources/Public/lib/images/marker-icon.png\');background-repeat: no-repeat;width: '+settings.pinW+'px;height: '+settings.pinH+'px;"><i></i></div>');
				} else {
					var elem = $(document.createElement('div')).addClass("marker").css({'width':settings.pinW,'height':settings.pinH}).prepend('<div class="marker" style="background-image: url(\'../typo3conf/ext/int_park/Resources/Public/lib/images/marker-icon.png\');background-repeat: no-repeat;width: '+settings.pinW+'px;height: '+settings.pinH+'px;"><i></i></div>');
				}
				$(elem).tooltip({
...
//			the note/marker elements
			...
			this.options.onClick = 	function(ev) {
										if (self.options.canEdit) {
											...
											if (rpos) {
												var elem = self.addNote({x: rpos.x, y: rpos.y, note: "", icon: ""});
